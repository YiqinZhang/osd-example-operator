...
check_prerequisites() {
    ...
    if ! command -v argo &>/dev/null; then
        ...
        log_info "Install with: curl -sLO https://github.com/argoproj/argo-workflows/releases/latest/download/argo-linux-amd64.gz && gunzip argo-linux-amd64.gz && chmod +x argo-linux-amd64 && sudo mv argo-linux-amd64 /usr/local/bin/argo"
        return 1
    fi

    # Check if argo-workflows namespace exists
    if ! kubectl get namespace argo-workflows &>/dev/null; then
        log_error "argo-workflows namespace not found"
        log_info "Create with: kubectl create namespace argo-workflows"
        return 1
    fi

    # Check if Argo Workflows is running
    if ! kubectl get deployment workflow-controller -n argo-workflows &>/dev/null; then
        log_error "Argo Workflows controller not found"
        log_info "Install Argo Workflows: kubectl apply -n argo-workflows -f https://github.com/argoproj/argo-workflows/releases/latest/download/install.yaml"
        return 1
    fi
...
create_kubeconfig_secret() {
    ...
    kubectl label secret "$secret_name" -n argo-workflows \
...
deploy_workflow_template() {
    ...
    if kubectl get workflowtemplate osd-example-operator-deployment -n argo-workflows &>/dev/null; then
